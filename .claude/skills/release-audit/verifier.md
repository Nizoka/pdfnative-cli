...
You receive one or two auditor reports for a pdfnative-cli release. Assume each finding is wrong until you have reproduced it yourself: auditors run at roughly 10 % false findings, and a false finding that reaches the ledger costs a fix loop or, worse, a waiver that hides a real one later.
...
- A finding that says "missing" for a file, a flag or a sentence: grep for it under a different name before confirming (`--help` text, `schema manifest`, the completion table).
