...
|---|---|---|---|---|---|---|
| A-03 | major | "render --pdfx pdfx4 writes a PDF/X-4 output intent" | `npx vitest run tests/commands/render-pdfx.test.ts -t "output intent"` | 2 passed | 2 passed | holds |
| B-07 | blocker | `schema status` pins the `pdfx` envelope field | `node dist/cli.cjs schema status \| node -e "..."` | absent | present | finding |
...
| … | stamp | justification |
|---|---|---|
| … | CONFIRMED | re-ran the command; `pdfx` is emitted at src/commands/render.ts:conformanceFields and absent from the status schema |
| … | DOWNGRADED → minor | real, but `flag-parity` fails the gate on it — cannot ship silently |
| … | REJECTED | the flag is documented under the Conformance block of RENDER_USAGE |
