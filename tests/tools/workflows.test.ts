...
// Ported from pdfnative's tests/tools/workflows.test.ts (1.8.0).
...
describe('ci.yml', () => {
    ...
    it('lists no gate step by hand', () => {
        for (const step of ['typecheck:all', 'test:coverage', 'validate:pdfa', 'validate:pdfx', 'verify:samples', 'verify:docs', 'npm run build', 'npm run lint', 'node dist/cli.cjs --help']) {
            expect(ci, step).not.toMatch(new RegExp(`run: (npm run )?${escapeRegExp(step)}\\s*$`, 'm'));
        }
    });
});
...
describe('publish.yml', () => {
    ...
    it('runs the publish gate with --require-all after veraPDF, and lists no gate step by hand', () => {
        ...
        for (const hand of ['npm run validate:pdfa', 'npm run corpus:pdfa', 'npm run verify:samples', 'npm run test:coverage', 'npm run typecheck:all', 'node dist/cli.cjs --help']) {
    ...
    it('has an attest job with exactly three permissions that attests the tarball and the SBOM', () => {
        ...
        expect(body).toMatch(/pdfnative-cli-\$\{VERSION\}\.tgz/);
...
describe('package.json publish settings', () => {
    ...
    it('keeps pdfnative as the only runtime dependency', () => {
        expect(Object.keys(pkg.dependencies)).toEqual(['pdfnative']);
    });
...
describe('sample-regression.yml', () => {
    ...
    it('builds the CLI before generating and verifying', () => {
