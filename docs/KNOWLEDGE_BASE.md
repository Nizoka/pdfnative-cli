...
| `formField` | `fieldType` (`text`\|`multilineText`\|`checkbox`\|`radio`\|`dropdown`\|`listbox` — any other value is refused with `E_INPUT`), `name` | `label`, `value`, `placeholder`, `options`, `readOnly`, `required`, `maxLength`, `width` | [01-contact-form.json](../samples/render/form/01-contact-form.json) |
