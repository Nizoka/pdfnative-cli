/**
 * pdfnative-cli — Sample fingerprinting core (v1.5.0)
 * ====================================================
 ...
 * "unchanged output" means. Ported from pdfnative's
 * scripts/lib/sample-fingerprint.ts (1.8.0); the corpus here is written by
 * the BUILT CLI (scripts/generate-samples.ts → test-output/samples/).
 *
 * Two modes:
 *   • `bytes`    — SHA-256 of the file. Valid for every sample that is
 *     byte-reproducible once the generator pins the creation instant
 *     (`--creation-date` + SOURCE_DATE_EPOCH) and runs in UTC
 *     (scripts/helpers/tz.ts); pdfnative 1.8.0 writes every date in UTC.
...
import { openPdf, extractText, listSignatures } from '../../src/core-bridge/index.js';
import type { PdfReader, PdfValue, PdfDict } from '../../src/core-bridge/index.js';
 ...
 * them (the encryption generator's deterministic PASSWORDS table).
 *
 ...
 * quietly switching fingerprint mode. Owner-only samples still need the
 * password here because the reader tries user and owner passwords alike.
 */
export const ENCRYPTED_SAMPLES: Readonly<Record<string, string>> = {
    'encryption/01-aes128-protected.pdf': 'sample-owner',
    'encryption/02-aes256-protected.pdf': 'sample-owner',
    'encrypt/document-aes128.pdf': 'sample-owner',
    'encrypt/document-aes256.pdf': 'sample-owner',
    'encrypt/rotated.pdf': 'rotated-owner',
    'page-tree/merged-encrypted.pdf': 'merged-owner',
};

/**
 * Signed samples are fingerprinted semantically too: the incremental
 * revision a signature appends carries a per-revision trailer /ID the engine
 * derives at signing time, so two signing runs of the same bytes differ
 * (observed with both crypto providers). The projection records what the
 * document says plus the signature inventory (field names, sub-filters,
 * byte-range shape), so a signing regression is still caught.
 */
...
export function semanticSamplePaths(): string[] {
    return [...Object.keys(ENCRYPTED_SAMPLES), ...SIGNED_SAMPLES].sort();
}
...
export function fingerprint(absPath: string, rel: string): Fingerprint {
    ...
    if (SIGNED_SAMPLES.includes(rel)) {
 ...
 * produced two ways (a streaming and an in-memory page-tree operation, a
 * document rendered through two equivalent flag spellings). Any other pair
 * sharing a hash is a showcase whose "with" and "without" variants collapsed
 * into the same file.
 */
export const IDENTICAL_SAMPLE_GROUPS: ReadonlyArray<ReadonlyArray<string>> = [
    ['page-tree/merged.pdf', 'page-tree/merged-stream.pdf'],
    // `extract --pages 1` and the first part of `split` rebuild the same page.
    ['page-tree/extract-p1.pdf', 'page-tree/split-1.pdf'],
    // The same document pinned by the flag and by SOURCE_DATE_EPOCH: identical by design.
    ['reproducible/01-pinned-date.pdf', 'reproducible/02-source-date-epoch.pdf'],
];
