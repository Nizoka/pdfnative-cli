#!/usr/bin/env tsx
/**
 * pdfnative-cli — Sample regression gate (v1.5.0)
 * =================================================
 * Fingerprints every generated sample and compares the result to the
 * committed baseline, so a change anywhere in the CLI — or in the pdfnative
 ...
 * every later rebaseline that leaves the sample alone, so 1.6.0 will still be
 * held to the bytes v1.5.0 emitted, and so on. An entry whose hash is
...
function saveBaseline(entries: Record<string, Fingerprint>, previous: Baseline | null): void {
    ...
    const payload: Baseline = {
        $comment:
            'Fingerprints of every sample in test-output/samples/ (written by the BUILT CLI, '
            ...
            + 'the rebaseline when the hash changes, so the chain 1.5.0 -> 1.6.0 -> … stays '
...
function main(): number {
    ...
    if (total === 0 && unreadable.length === 0) {
        console.error('test-output/samples/ holds no PDFs — run `npm run build && npm run test:generate` first.');
        return 2;
    }
