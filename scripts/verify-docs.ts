#!/usr/bin/env tsx
/**
 * pdfnative-cli — documentation consistency verifier (ported from pdfnative 1.8.0)
 * ================================================================================
 * Every version string, command count, subject count and error-code list
 * quoted in the docs was hand-copied across a dozen files — README, the
 * knowledge base, llms.txt, AGENTS.md, the Copilot instructions, SECURITY.md,
 * CITATION.cff — and nothing checked them: 1.4.0 shipped with the Copilot
 * file still saying "eleven commands" (real: 21) and "Node ≥ 20" (real: 22).
 *
 ...
 * source constants the CLI runs on (`scripts/lib/cli-surface.ts`), and fails
...
/** Every stable error code must be listed, by name, in each of these. */
export const ERROR_CODE_DOCS: readonly string[] = ['README.md', 'llms.txt', 'docs/KNOWLEDGE_BASE.md', 'docs/AGENT_CONTRACT.md'];

export const OFFLINE_RULES = [
    'manifest-shape', // manifest fields are well-formed; the pdfnative pin and the CLI version agree with package.json; assertions' expectFrom agrees with declared/derived
    'derived-counts', // derived.* equals the source constants and the tree (commands, subjects, codes, flags, corpus, tests, samples, baseline)
    ...
    'count-tokens', // tests / files / samples / scripts / coverage / "Current version" tokens equal the manifest
    'command-parity', // COMMANDS == USAGE list == both index.ts switches == README reference == manifest commandGroups; samples/ dirs are commands
 ...
 * release with 12 commands"). Rewriting those would falsify the roadmap, so
 * a line may opt out with `verify-docs:allow <rule>` on itself or on the
...
export async function verifyDocs(root: string, options: VerifyOptions = {}): Promise<VerifyResult> {
    ...
    const cliVersion = manifest.packages['pdfnative-cli']?.version ?? '';
    const coreVersion = manifest.packages['pdfnative']?.version ?? '';

    // ── The documentation corpus ──────────────────────────────────
    const DOC_FILES: string[] = [
        ...['README.md', 'ROADMAP.md', 'AGENTS.md', 'CLAUDE.md', 'CONTRIBUTING.md', 'SECURITY.md', 'llms.txt',
            '.github/copilot-instructions.md', '.github/AGENT_RULES.md', 'samples/README.md', 'tests/fixtures/README.md',
            `release-notes/v${cliVersion}.md`]
            .map((f) => join(root, f))
            .filter(existsSync),
    ...
    if (pkgJsonText !== null) {
        const pkgJson = JSON.parse(pkgJsonText) as { version?: string; dependencies?: Record<string, string>; engines?: { node?: string }; packageManager?: string };
        if (pkgJson.version !== cliVersion) {
            fail(MANIFEST_REL, 1, 'manifest-shape', `packages.pdfnative-cli.version is ${cliVersion} but package.json says ${pkgJson.version} — run release:prepare`);
        }
        ...
        if (pinFloor !== undefined && pinFloor !== coreVersion) {
            fail(MANIFEST_REL, 1, 'manifest-shape', `packages.pdfnative.version is ${coreVersion} but the dependency floor is ${pinFloor} — the manifest names the engine the CLI is built on`);
        }
    }
    ...
    const KNOWN_DERIVED = new Set([...Object.keys(actual), 'commandGroups', '$comment']);
    for (const key of Object.keys(manifest.derived)) {
        if (!KNOWN_DERIVED.has(key)) {
            fail(MANIFEST_REL, 1, 'manifest-shape', `derived.${key} is not computed by any rule — typo, or add it to scripts/lib/cli-surface.ts computeDerived()`);
        }
    }
    for (const [key, value] of Object.entries(actual)) {
        // The font tables are declared (they are pdfnative's inventory) but
        // the tree can check them: the allow-list in src/utils/fonts.ts.
        const declaredHere = key === 'bundledFontModules' || key === 'unicodeScripts';
        ...
        if (stated === undefined) {
            if (!declaredHere && key !== 'fontAliases') fail(MANIFEST_REL, 1, 'derived-counts', `derived.${key} is missing — the tree has ${value}`);
            continue;
        }
        if (stated !== value) {
    ...
    {
        const pdfa = CORPUS.filter((e) => e.claims === 'pdfa').length;
        const pdfx = CORPUS.filter((e) => e.claims === 'pdfx').length;
        if (manifest.declared['pdfaSamples'] !== pdfa) fail(MANIFEST_REL, 1, 'derived-counts', `declared.pdfaSamples says ${String(manifest.declared['pdfaSamples'])} but the corpus table claims PDF/A for ${pdfa} files`);
        if (manifest.declared['pdfxSamples'] !== pdfx) fail(MANIFEST_REL, 1, 'derived-counts', `declared.pdfxSamples says ${String(manifest.declared['pdfxSamples'])} but the corpus table claims PDF/X for ${pdfx} files`);
    }
    {
    ...
    // ── Rule: count-tokens ────────────────────────────────────────
    {
        ...
        const COUNT_TOKENS: readonly CountToken[] = [
            ...
            { pattern: /\b(\d+)\s+global flags\b/g, source: 'derived.globalFlags', mode: 'equal' },
            { pattern: /\b(\d+)\s+(?:bundled\s+)?font modules\b/g, source: 'declared.bundledFontModules', mode: 'equal' },
            ...
            { pattern: /\b(\d+)\s+(?:conformance\s+)?corpus files\b/g, source: 'derived.pdfaCorpus', mode: 'equal' },
            ...
            // "27 Unicode scripts", "27 script codes", "27-script" — but never
            // "63 sample scripts" or "shell scripts" (the runnable demos), the
            // trailing "0" of "1.3.0 scripts", or a parenthesised grouping
            // such as "Indic (9 scripts)".
            ...
            { pattern: /(?<![\d.(])(?<!sample )(?<!shell )(?<!driver )\b(\d+)-scripts?\b/gi, source: 'declared.unicodeScripts', mode: 'equal' },
        ];
        ...
        // "Current version: X.Y.Z" — llms.txt names the CLI version in prose
        // without the package name beside it, so version-token cannot see it.
        ...
        for (const file of DOC_FILES) {
            ...
            while ((m = CURRENT_VERSION.exec(text)) !== null) {
                if (m[1] === cliVersion) continue;
                const line = lineOf(text, m.index);
                if (isSuppressed(lines, line, 'count-tokens') || isSuppressed(lines, line, 'version-token')) continue;
                fail(rel(file), line, 'count-tokens', `"${m[0].trim()}" — the manifest says pdfnative-cli is ${cliVersion}`);
            }
        }
    }
    ...
    // ── Rule: command-parity ──────────────────────────────────────
    {
        ...
        const indexText = readOr('src/index.ts');
        if (indexText === null) {
            fail('src/index.ts', 1, 'command-parity', 'missing — the dispatcher is the command surface');
        } else {
            const usage = usageCommandList(indexText);
            if (usage.declared !== table.length) fail('src/index.ts', 1, 'command-parity', `USAGE says "Commands (${usage.declared ?? '?'})" but the completion table has ${table.length}`);
            if (!same(usage.commands, table)) {
                const missing = table.filter((c) => !usage.commands.includes(c));
                const extra = usage.commands.filter((c) => !tableSet.has(c));
                fail('src/index.ts', 1, 'command-parity', `USAGE command list differs from completion.ts COMMANDS${missing.length ? ` — not listed: ${missing.join(', ')}` : ''}${extra.length ? ` — listed but not in the table: ${extra.join(', ')}` : ''}`);
            }
            const cases = switchCaseCounts(indexText);
            for (const c of table) {
                const n = cases.get(c) ?? 0;
                if (n !== 2) fail('src/index.ts', 1, 'command-parity', `"${c}" appears in ${n} switch case(s) — the --help switch and loadCommand() must both know it`);
            }
            for (const c of cases.keys()) {
                if (!tableSet.has(c)) fail('src/index.ts', 1, 'command-parity', `switch case "${c}" is not in completion.ts COMMANDS — completions and the manifest cannot see it`);
            }
        }
        const readme = readOr('README.md');
        if (readme !== null) {
            const headings = readmeCommandHeadings(readme);
            for (const c of table) if (!headings.includes(c)) fail('README.md', 1, 'command-parity', `no "### \`pdfnative ${c}\`" reference section`);
            for (const h of headings) if (!tableSet.has(h)) fail('README.md', 1, 'command-parity', `reference section "### \`pdfnative ${h}\`" names no command`);
        }
        const groups = manifest.derived['commandGroups'];
        if (groups && typeof groups === 'object') {
            const grouped = Object.values(groups as Record<string, string[]>).flat();
            if (!same(grouped, table)) fail(MANIFEST_REL, 1, 'command-parity', `derived.commandGroups lists ${grouped.length} commands (${grouped.filter((c) => !tableSet.has(c)).join(', ') || 'none unknown'}) but the completion table has ${table.length}`);
            if (manifest.derived['commands'] !== grouped.length) fail(MANIFEST_REL, 1, 'command-parity', `derived.commands is ${String(manifest.derived['commands'])} but commandGroups lists ${grouped.length}`);
        } else {
            fail(MANIFEST_REL, 1, 'command-parity', 'derived.commandGroups is missing — the README and llms.txt group the commands by it');
        }
        for (const dir of sampleDirs(root)) {
            if (!tableSet.has(dir) && !NON_COMMAND_SAMPLE_DIRS.includes(dir)) {
                fail(`samples/${dir}`, 1, 'command-parity', `is neither a command nor one of ${NON_COMMAND_SAMPLE_DIRS.join(', ')} — a renamed command left its samples behind`);
            }
        }
    }
    ...
    // ── Rule: flag-parity ─────────────────────────────────────────
    {
        const indexText = readOr('src/index.ts');
        if (indexText !== null) {
            for (const d of flagDrift(indexText)) {
                if (d.usageOnly.length > 0) fail('src/index.ts', 1, 'flag-parity', `${d.command.toUpperCase().replace(/-/g, '_')}_USAGE names ${d.usageOnly.join(', ')}, which no completion entry declares — add the flag to COMMANDS in src/commands/completion.ts`);
                if (d.completionOnly.length > 0) fail('src/commands/completion.ts', 1, 'flag-parity', `"${d.command}" declares ${d.completionOnly.join(', ')}, which its usage text never mentions — document the flag in ${d.command.toUpperCase().replace(/-/g, '_')}_USAGE`);
            }
        }
    }
    ...
    // ── Rule: schema-parity ───────────────────────────────────────
    {
        const kb = readOr('docs/KNOWLEDGE_BASE.md');
        if (kb !== null) {
            const start = kb.indexOf('### `schema` command');
            const section = start === -1 ? '' : kb.slice(start, kb.indexOf('\n## ', start) === -1 ? undefined : kb.indexOf('\n## ', start));
            if (start === -1) fail('docs/KNOWLEDGE_BASE.md', 1, 'schema-parity', 'has no "### `schema` command" section listing the subjects');
            for (const subject of schemaSubjects()) {
                if (!section.includes(`\`${subject}\``)) fail('docs/KNOWLEDGE_BASE.md', start === -1 ? 1 : lineOf(kb, start), 'schema-parity', `schema subject "${subject}" is not listed in the schema section`);
            }
        }
    }
    ...
    // ── Rule: error-parity ────────────────────────────────────────
    {
        const codes = errorCodeValues();
        const codeSet = new Set(codes);
        const agent = readOr('src/utils/agent.ts');
        if (agent !== null) {
            const keys = defaultMessageKeys(agent);
            const expected = errorCodeKeys();
            for (const k of expected) if (!keys.includes(k)) fail('src/utils/agent.ts', 1, 'error-parity', `DEFAULT_MESSAGE has no entry for ErrorCode.${k}`);
            for (const k of keys) if (!expected.includes(k)) fail('src/utils/agent.ts', 1, 'error-parity', `DEFAULT_MESSAGE names ErrorCode.${k}, which src/utils/error.ts does not declare`);
        }
        for (const file of DOC_FILES) {
            const text = texts.get(file)!;
            for (const token of errorTokens(text)) {
                if (codeSet.has(token)) continue;
                const at = text.indexOf(token);
                const lines = text.split(/\r?\n/);
                const line = lineOf(text, at);
                if (isSuppressed(lines, line, 'error-parity')) continue;
                fail(rel(file), line, 'error-parity', `"${token}" is not a stable error code (src/utils/error.ts declares ${codes.join(', ')})`);
            }
        }
        for (const doc of ERROR_CODE_DOCS) {
            const text = readOr(doc);
            if (text === null) {
                fail(doc, 1, 'error-parity', 'missing — it must list every stable error code by name');
                continue;
            }
            const present = new Set(errorTokens(text));
            const absent = codes.filter((c) => !present.has(c));
            if (absent.length > 0) fail(doc, 1, 'error-parity', `does not name ${absent.join(', ')} — every stable error code is part of the contract`);
        }
    }

    // ── Rule: governance-embed ────────────────────────────────────
    {
        const text = readOr('.github/ai-governance.json');
        if (text === null) {
            fail('.github/ai-governance.json', 1, 'governance-embed', 'missing — `pdfnative govern policy` embeds it and agents read it');
        } else {
            try {
                if (canonicalJson(JSON.parse(text)) !== canonicalJson(AI_GOVERNANCE_POLICY)) {
                    fail('.github/ai-governance.json', 1, 'governance-embed', 'differs from AI_GOVERNANCE_POLICY in src/utils/governance.ts — the file and the embedded copy `pdfnative govern policy` prints must be identical');
                }
            } catch (err) {
    ...
    // ── Rule: sample-shell-parity ─────────────────────────────────
    for (const orphan of unpairedSampleScripts(root)) {
        fail(orphan, 1, 'sample-shell-parity', `has no ${orphan.endsWith('.sh') ? '.ps1' : '.sh'} twin — every sample runs under both Git Bash and PowerShell`);
    }
