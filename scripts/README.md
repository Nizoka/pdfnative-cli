...
| `generate-pdfa-corpus.ts` | `corpus:pdfa` | `corpus:pdfa` | Writes the 22-file PDF/A + PDF/X conformance corpus (`lib/pdfa-corpus.ts`) into `test-output/pdfa/` through the built CLI | `--quiet`, `--verbose`, `--json` | 0 written, 1 a CLI invocation failed, 2 `dist/cli.cjs` missing or bad usage |
